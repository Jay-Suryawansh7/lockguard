...
export function calcRiskScore(cveResults, hookResults, diff) {
    let score = 0;
...
export function reportText(result) {
    ...
    if (result.diff) {
        ...
        lines.push('\n─── Lockfile Changes ───');
        if (d.totalAdded > 0)
            lines.push(`  Added: ${d.totalAdded}`);
        if (d.totalChanged > 0)
            lines.push(`  Changed: ${d.totalChanged}`);
        if (d.totalRemoved > 0)
            lines.push(`  Removed: ${d.totalRemoved}`);
    }
    lines.push('');
    return lines.join('\n');
}
